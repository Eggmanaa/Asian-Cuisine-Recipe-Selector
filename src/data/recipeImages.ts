// Recipe Images Map
// Maps recipe IDs to their image URLs

export const recipeImages: { [key: string]: string } = {
  // Thai Recipes
  'pad-thai': 'https://cdn1.genspark.ai/user-upload-image/5_generated/7f52c9d3-17ca-4aa2-a153-d8e34bfef68a',
  'pad-see-ew': 'https://cdn1.genspark.ai/user-upload-image/5_generated/67b2bf1f-0ac6-4b47-a1a6-787346d27d01',
  'cashew-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/bf466d89-e9bc-4423-85fb-49339e58bc42',
  'massaman-curry': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ca42899f-00ab-466a-ac7f-390b4228be2b',
  'tom-kha-gai': 'https://cdn1.genspark.ai/user-upload-image/5_generated/b0c4f164-67ab-48d0-8aa4-7970ba59a43c',
  'pad-woon-sen': 'https://cdn1.genspark.ai/user-upload-image/5_generated/1196c054-1bc3-49c8-addd-8d405eb30dd4',
  'penang-curry': 'https://cdn1.genspark.ai/user-upload-image/5_generated/20197c67-4ab5-4935-b7b9-29d55d3ed132',
  
  // Chinese Recipes
  'hong-shao-rou': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ab19493f-6ffb-45e3-a35c-2118d7527ae4',
  'garlic-eggplant': 'https://cdn1.genspark.ai/user-upload-image/5_generated/9c192991-7287-43dc-aa8f-e7b87dc249da',
  'kung-pao-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/c9cf0282-4e8b-48e5-ade0-710e6faf486a',
  'char-siu': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ea4f21de-ee90-4400-9377-573a2de5bd9c',
  'singapore-noodles': 'https://cdn1.genspark.ai/user-upload-image/5_generated/609042c7-9d35-4262-9085-05862861584b',
  'three-cup-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/a40e338c-cf5b-486e-875b-6d9a0ef78706',
  'mapo-tofu': 'https://cdn1.genspark.ai/user-upload-image/5_generated/6c52d3c4-d9be-4a89-90d5-4ebf4a6fdc6c',
  'wonton-soup': 'https://cdn1.genspark.ai/user-upload-image/5_generated/bec45414-3903-4689-85a6-bf8cf235c786',
  'laksa': 'https://cdn1.genspark.ai/user-upload-image/5_generated/3e197e95-7ff9-4c4b-89ef-a16b95242640',
  'dan-dan-noodles': 'https://cdn1.genspark.ai/user-upload-image/5_generated/b0187ded-c4f9-40ab-bdef-b51b6002661f',
  
  // Vietnamese Recipes
  'pho-bo': 'https://cdn1.genspark.ai/user-upload-image/5_generated/cca9480b-e75e-4539-93c4-587e280b7feb',
  'spring-rolls': 'https://cdn1.genspark.ai/user-upload-image/5_generated/d9e12f5f-4d39-4c8f-be7a-e605cf0e3fbf',
  'shaking-beef': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ebbe99c7-0416-44f4-a0b5-689d1b8b8085',
  'bun-thit-nuong': 'https://cdn1.genspark.ai/user-upload-image/5_generated/d494df03-4281-42e5-a471-d55d562a532f',
  'bo-kho': 'https://cdn1.genspark.ai/user-upload-image/5_generated/b3339f5c-06a4-41bb-aa72-8b0af672d18f',
  
  // Indian Recipes
  'butter-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/e293d74d-a096-4189-9542-e14d4272e852',
  'tandoori-lamb': 'https://cdn1.genspark.ai/user-upload-image/5_generated/5ac683b0-e36f-4e10-81cc-c8d9566e1e69',
  'chicken-saag': 'https://cdn1.genspark.ai/user-upload-image/5_generated/72e08d2c-e736-40f9-ae6f-79242cbb89ff',
  'chicken-tikka-masala': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ad81fc96-9b6c-482c-ab1c-5e7e72e58e8f',
  'lamb-pasanda': 'https://cdn1.genspark.ai/user-upload-image/5_generated/2a0260d0-63a2-4043-924c-ec46f4eb712c',
  'vegetable-korma': 'https://cdn1.genspark.ai/user-upload-image/5_generated/b2bf32de-1365-49a6-927f-61629f8b585e',
  'chicken-korma': 'https://cdn1.genspark.ai/user-upload-image/5_generated/3e224f23-0927-4ede-a1a0-235b183aa7ad',
  
  // Indonesian
  'beef-rendang': 'https://cdn1.genspark.ai/user-upload-image/5_generated/0647a088-89f6-4f88-a6ba-d7642b777aee',
  
  // Korean
  'bibimbap': 'https://cdn1.genspark.ai/user-upload-image/5_generated/604385b8-3bd7-4da5-81b9-485541e0f9ae',
  
  // Other
  'tom-yum': 'https://cdn1.genspark.ai/user-upload-image/5_generated/605f25af-2a0d-4461-8c04-65a601318bf1',
  'katsu-curry': 'https://cdn1.genspark.ai/user-upload-image/5_generated/d8231f87-028e-4f0b-825c-748f880f73d3',
  
  // New Chinese Recipes
  'bang-bang-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/42472fa3-1595-4e0d-b814-047cdbbde0d9',
  'tomato-scrambled-eggs': 'https://cdn1.genspark.ai/user-upload-image/5_generated/4ca078a7-1ff8-46d3-96ea-156db991ce7f',
  'twice-cooked-pork': 'https://cdn1.genspark.ai/user-upload-image/5_generated/ef43dfaf-4ac4-424d-8f0a-89d0094300e2',
  'sweet-sour-ribs': 'https://cdn1.genspark.ai/user-upload-image/5_generated/bfad2cb6-1e83-4b0b-975f-d48e66dc64c5',
  'mala-chicken': 'https://cdn1.genspark.ai/user-upload-image/5_generated/71fb0733-420e-491a-902e-cc27708c9f84',
  'dandan-noodles': 'https://cdn1.genspark.ai/user-upload-image/5_generated/b37d2526-4cb4-4efa-802f-adac9669e093',
  'fish-fragrant-eggplant': 'https://cdn1.genspark.ai/user-upload-image/5_generated/a3521a37-0332-44b1-bb06-243fe114b2e5',
  'guobaorou': 'https://cdn1.genspark.ai/user-upload-image/5_generated/38920990-a254-480b-a7ab-18ff319febb1',
  
  // Latest Chinese Recipes (Second Batch)
  'black-pepper-beef': 'https://cdn1.genspark.ai/user-upload-image/5_generated/35c4f886-dfe9-4745-a14f-a770cad82da9',
  'home-style-tofu': 'https://cdn1.genspark.ai/user-upload-image/5_generated/bcd7b0e4-e7b9-4f73-b7ce-4e41901cdc9f',
  'cumin-lamb': 'https://cdn1.genspark.ai/user-upload-image/5_generated/0150be1e-5e22-469b-b733-39b65431bbd3',
  'squirrel-fish': 'https://cdn1.genspark.ai/user-upload-image/5_generated/aa8594b8-5060-4d63-ba7f-0a6566d47956',
  
  // Korean Recipes (New Additions)
  'bulgogi': 'https://cdn1.genspark.ai/user-upload-image/5_generated/db1ce9ff-cb90-4709-b838-69587074ca1d',
  'japchae': 'https://cdn1.genspark.ai/user-upload-image/5_generated/14f253aa-c5e0-4754-8731-c8217cbbbae3'
};

// Fallback image for recipes without specific images yet
export const fallbackImage = 'data:image/svg+xml;base64,';

// Get image URL for a recipe
export function getRecipeImage(recipeId: string): string {
  return recipeImages[recipeId] || fallbackImage;
}